...
public class AdminHeartAdjustmentService {
  ...
  public AdminHeartView findHearts(Long userId) {
    ...
    int balance = heartWalletRepository.findByUserId(userId).map(HeartWallet::getBalance).orElse(0);
    List<AdminHeartTransactionView> transactions =
        heartTransactionRepository.findByUserIdOrderByCreatedAtDesc(userId).stream()
            .limit(10)
    ...
    if (command.type() == AdminHeartAdjustmentType.CHARGE) {
      ...
      transaction =
          HeartTransaction.charge(
              userId, command.amount(), wallet.getBalance(), ADMIN_ADJUSTMENT_REFERENCE, adminId);
  ...
  public record AdminHeartAdjustmentCommand(
      AdminHeartAdjustmentType type, Integer amount, String reason) {}

  public record AdminHeartView(
      Long userId, int balance, List<AdminHeartTransactionView> transactions) {}
