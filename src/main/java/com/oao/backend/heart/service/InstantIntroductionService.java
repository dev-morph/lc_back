...
public class InstantIntroductionService {
  ...
  public InstantIntroductionCost currentCost(Long userId) {
    ...
    return usageWindowRepository
        .findTopByUserIdOrderByWindowStartedAtDesc(userId)
        .filter(window -> !window.isExpired(now))
        .map(
            window ->
                new InstantIntroductionCost(
  ...
  public InstantIntroductionResult request(Long userId) {
    ...
    Instant now = Instant.now();
    InstantIntroductionConfig config = costPolicy.currentConfig();
    InstantIntroUsageWindow window =
        usageWindowRepository
            .findTopByUserIdOrderByWindowStartedAtDesc(userId)
            .filter(activeWindow -> !activeWindow.isExpired(now))
            .orElseGet(
                () ->
                    usageWindowRepository.save(
                        InstantIntroUsageWindow.start(userId, now, config.getUsageWindowHours())));
    ...
    heartTransactionRepository.save(
        HeartTransaction.spend(
            userId, cost, wallet.getBalance(), "INSTANT_INTRODUCTION", proposal.getId()));
    notificationService.notifyMatchArrived(
        proposal.getId(), requester.userId(), counterpart.userId());

    return new InstantIntroductionResult(
        cost,
        wallet.getBalance(),
        window.getUsageCount(),
        window.getWindowExpiresAt(),
        proposal.getId(),
        true);
  }
  ...
  private ScoredCounterpart findBestCounterpart(MatchingCandidate requester) {
    ...
    return userAccountRepository.findAll().stream()
        ...
        .filter(candidate -> matchingPolicy.pairAllowed(requester.userId(), candidate.userId()))
        .filter(
            candidate ->
                !matchProposalRepository.existsActivePair(
                    requester.userId(), candidate.userId(), ACTIVE_MATCH_STATUSES))
        .map(
            candidate ->
                new ScoredCounterpart(
                    candidate,
                    matchingScoreCalculator.calculate(requester, candidate, scoreConfig)))
        .max(
  ...
  public record InstantIntroductionCost(
      int requiredHearts, int nextUsageCount, Instant windowExpiresAt) {}
