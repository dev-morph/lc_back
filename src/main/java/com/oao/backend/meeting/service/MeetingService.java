...
public class MeetingService {
  ...
  public List<MeetingView> findMeetings(Long userId, String tabValue) {
    ...
    List<MeetingApplication> applications =
        meetingApplicationRepository.findByUserIdOrderByCreatedAtDescIdDesc(userId);
    Map<Long, MeetingApplication> applicationByEventId =
        applications.stream()
            .collect(
                Collectors.toMap(
                    MeetingApplication::getMeetingEventId,
                    Function.identity(),
                    (left, right) -> left));

    if (tab == MeetingTab.ONGOING) {
      return meetingEventRepository
          .findByStatusOrderByEventDateTimeAscIdAsc(MeetingEventStatus.OPEN)
          .stream()
    ...
    List<MeetingApplication> filteredApplications =
        applications.stream().filter(application -> tab.matches(application)).toList();
    Map<Long, MeetingEvent> eventById =
        meetingEventRepository
            .findAllById(
                filteredApplications.stream().map(MeetingApplication::getMeetingEventId).toList())
            .stream()
            .collect(Collectors.toMap(MeetingEvent::getId, Function.identity()));
  ...
  public MeetingView findMeeting(Long meetingId, Long userId) {
    ...
    if (event.getStatus() == MeetingEventStatus.DRAFT)
      throw new BusinessException(HttpStatus.NOT_FOUND, "Meeting was not found.");
    MeetingApplication application =
        meetingApplicationRepository.findByMeetingEventIdAndUserId(meetingId, userId).orElse(null);
    return toView(event, application);
  }

  @Transactional
  public MeetingView apply(Long meetingId, Long userId) {
    db.jdbc.queryForList("select id from meeting_event where id=? for update", meetingId);
    MeetingEvent event = findEvent(meetingId);
    if (event.getStatus() != MeetingEventStatus.OPEN
        || event
            .getEventDateTime()
            .isBefore(LocalDateTime.now(java.time.ZoneId.of("Asia/Seoul")))) {
      throw new BusinessException(
          HttpStatus.BAD_REQUEST, "This meeting is not open for applications.");
    }
    if (meetingApplicationRepository.findByMeetingEventIdAndUserId(meetingId, userId).isPresent()) {
      throw new BusinessException(
          HttpStatus.BAD_REQUEST, "You have already applied to this meeting.");
    }

    MeetingApplication application =
        meetingApplicationRepository.save(MeetingApplication.apply(meetingId, userId));
    return toView(event, application);
  }
  ...
  private ParticipationStatus participationStatus(MeetingApplication application) {
    ...
    if (application.getApplicationStatus() == MeetingApplicationStatus.APPROVED
