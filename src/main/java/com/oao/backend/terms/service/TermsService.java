...
public class TermsService {
  ...
  public TermsAgreementStatus agreementStatus(Long userId) {
    ...
    List<Long> requiredTermIds = currentRequiredTerms.stream().map(TermsDocument::getId).toList();
    Set<Long> agreedTermIds = agreedTermIds(userId, requiredTermIds);
    List<TermsDocument> missingRequiredTerms =
        currentRequiredTerms.stream()
  ...
  public TermsAgreementStatus agree(
      Long userId, List<TermsAgreementCommand> commands, String ipAddress, String userAgent) {
        ...
        .ifPresent(
            command -> {
              throw new BusinessException(
                  HttpStatus.BAD_REQUEST, "Only agreed terms can be saved.");
            });

    List<Long> requestedTermIds =
        commands.stream().map(TermsAgreementCommand::termsDocumentId).distinct().toList();
    Map<Long, TermsDocument> termsDocumentById =
        termsDocumentRepository.findAllById(requestedTermIds).stream()
            .collect(Collectors.toMap(TermsDocument::getId, Function.identity()));

    List<Long> missingTermIds =
        requestedTermIds.stream()
    ...
    List<UserTermsAgreement> newAgreements =
        requestedTermIds.stream()
            .filter(termsDocumentId -> !alreadyAgreedTermIds.contains(termsDocumentId))
            .map(
                termsDocumentId ->
                    UserTermsAgreement.agree(
                        userId, termsDocumentById.get(termsDocumentId), ipAddress, userAgent))
            .toList();
  ...
  public record TermsAgreementStatus(
      boolean requiredTermsAgreed, List<TermsDocument> missingRequiredTerms) {}
