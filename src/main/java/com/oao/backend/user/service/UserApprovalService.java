...
public class UserApprovalService {
  ...
  private String accountEmail(Long id, OAuthAccount oauth) {
    ...
    return emails.isEmpty()
        ? (oauth == null ? null : oauth.getEmail())
  ...
  public List<PendingApprovalUserView> findPendingUserViews() {
    return findPendingUsers().stream().map(this::toPendingApprovalUserView).toList();
  }

  @Transactional(readOnly = true)
  public List<AdminUserSearchView> searchUsers(AdminUserSearchCommand command) {
    AdminUserSearchCommand normalizedCommand =
        command == null ? AdminUserSearchCommand.empty() : command;
  ...
  public AdminUserSearchView createAdminUser(
    ...
    userGradeHistoryRepository.save(
        UserGradeHistory.record(
            user.getId(), previousGrade, command.grade(), adminId, "관리자 유저 생성"));
  ...
  public AdminUserDetailView findAdminUserDetail(Long userId) {
    ...
    ProfilePhoto photo =
        profilePhotoRepository.findFirstByUserIdOrderByDisplayOrderAscIdAsc(userId).orElse(null);
    OAuthAccount oauthAccount = oAuthAccountRepository.findFirstByUserId(userId).orElse(null);

    return new AdminUserDetailView(
        user.getId(),
        accountEmail(user.getId(), oauthAccount),
        user.getName(),
        user.getBirthDate(),
  ...
  public AdminUserSearchView deleteAdminUser(Long userId) {
    UserAccount user = findUser(userId);
    accountSettings.withdraw(userId);
    entityManager.clear();
    return toAdminUserSearchView(findUser(userId));
  }
  ...
  public UserAccount approve(Long userId, MemberGrade grade, Long adminId, String reason) {
    ...
    userGradeHistoryRepository.save(
        UserGradeHistory.record(userId, previousGrade, grade, adminId, reason));
    return user;
  }

  @Transactional
  public UserAccount reject(Long userId, String reason) {
    UserAccount user = findUser(userId);
    user.reject(reason);
    matchingProfileRepository.findByUserId(userId).ifPresent(MatchingProfile::pauseMatching);
    return user;
  }
  ...
  private PendingApprovalUserView toPendingApprovalUserView(UserAccount user) {
    ...
    MatchingProfile matchingProfile =
        matchingProfileRepository.findByUserId(user.getId()).orElse(null);
    ProfilePhoto photo =
        profilePhotoRepository
            .findFirstByUserIdOrderByDisplayOrderAscIdAsc(user.getId())
            .orElse(null);
  ...
  private AdminUserSearchView toAdminUserSearchView(
      UserAccount user, UserProfile profile, OAuthAccount oauthAccount) {
  ...
  private boolean matchesSearch(UserAccount user, AdminUserSearchCommand command) {
    ...
    if (command.approvalStatus() != null
        && !Objects.equals(enumName(user.getApprovalStatus()), command.approvalStatus())) {
      return false;
    }
  ...
  private void validateCreateCommand(
      AdminUserCreateCommand command, List<MultipartFile> photos, MultipartFile legacyPhoto) {
    if (command == null) {
    ...
    if (command.heightCm() == null || command.heightCm() < 120 || command.heightCm() > 230) {
      throw new BusinessException(
          HttpStatus.BAD_REQUEST, "Height must be between 120cm and 230cm.");
    }
    if (normalize(command.activityRegion()).length() < 2) {
      throw new BusinessException(
          HttpStatus.BAD_REQUEST, "Activity region must be at least 2 characters.");
    }
    if (normalize(command.education()).length() < 2) {
      throw new BusinessException(
          HttpStatus.BAD_REQUEST, "Education must be at least 2 characters.");
    }
    ...
    if (uploadedPhotoCount(photos, legacyPhoto) < ProfileIntroPhotoService.MIN_PHOTO_COUNT) {
      throw new BusinessException(
          ...
          "Profile photos must be at least " + ProfileIntroPhotoService.MIN_PHOTO_COUNT + ".");
    }
  ...
  private int uploadedPhotoCount(List<MultipartFile> photos, MultipartFile legacyPhoto) {
    int photoCount =
        photos == null
            ? 0
            : (int) photos.stream().filter(photo -> photo != null && !photo.isEmpty()).count();
  ...
  private List<Hobby> selectedHobbies(List<String> hobbyNames) {
    ...
    Map<String, Hobby> hobbyByName =
        hobbies.stream().collect(Collectors.toMap(Hobby::getName, Function.identity()));
