...
public class AppNotificationService {
  @org.springframework.beans.factory.annotation.Autowired private AlimtalkOutboxService outbox;
  ...
  public AppNotificationView markRead(Long notificationId, Long userId) {
    AppNotification notification =
        notificationRepository
            ...
            .orElseThrow(
                () -> new BusinessException(HttpStatus.NOT_FOUND, "Notification not found."));
  ...
  public void notifyMessageReceived(
      Long roomId, Long messageId, Long receiverUserId, Long senderUserId) {
    if (db.count(
            "select count(*) from user_preferences where user_id=? and message_notifications=false",
            receiverUserId)
        > 0) return;
    createAndPush(
        receiverUserId,
  ...
  private void createOnce(
      ...
      String linkUrl,
      Long actorUserId) {
    if (notificationRepository.existsByUserIdAndNotificationTypeAndTargetTypeAndTargetId(
    ...
    AppNotification notification =
        notificationRepository.save(
            AppNotification.create(
                userId, type, title, body, iconType, targetType, targetId, linkUrl, actorUserId));
    outbox.enqueue(userId, type.name(), targetId, linkUrl);
    var view = toView(notification);
    com.oao.backend.common.AfterCommit.run(
        () -> messagingTemplate.convertAndSend("/topic/users." + userId + ".notifications", view));
  }
