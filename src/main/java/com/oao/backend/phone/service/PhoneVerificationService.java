...
public class PhoneVerificationService {
  ...
  private com.oao.backend.config.DevToolsProperties devTools;

  @org.springframework.beans.factory.annotation.Autowired
  private com.oao.backend.common.UserLocks locks;
  ...
  public PhoneVerificationService(
      ...
      @Value("${oao.verification.message.dev-code-response-enabled:false}")
          boolean devCodeResponseEnabled) {
  ...
  public PhoneVerificationStatusView status(Long userId) {
    ...
    boolean verified =
        profile != null && profile.getPhoneNumber() != null && profile.getPhoneVerifiedAt() != null;
  ...
  public SendPhoneVerificationResult send(Long userId, String rawPhoneNumber) {
    ...
    if (!sendResult.sent() && !(devCodeResponseEnabled && devTools.available()))
      throw new BusinessException(
          HttpStatus.SERVICE_UNAVAILABLE, "문자 인증 서비스 설정이 필요합니다. 운영자에게 문의해주세요.");

    return new SendPhoneVerificationResult(
        verification.getId(),
        verification.getExpiresAt(),
        now.plus(RESEND_COOLDOWN),
        devCodeResponseEnabled && devTools.available() ? code : null,
        sendResult.message());
  }
  ...
  public ConfirmPhoneVerificationResult confirm(Long userId, Long verificationId, String code) {
    ...
    PhoneVerificationCode verification =
        verificationRepository
            ...
            .orElseThrow(
                () -> new BusinessException(HttpStatus.NOT_FOUND, "Verification not found."));
    ...
    return new ConfirmPhoneVerificationResult(
        true, maskPhone(verification.getPhoneNumber()), "/terms/onboarding");
  }

  private void enforceSendLimit(Long userId, String phoneNumber, Instant now) {
    verificationRepository
        .findFirstByUserIdAndStatusOrderByCreatedAtDescIdDesc(
            userId, PhoneVerificationStatus.PENDING)
        .ifPresent(
            latest -> {
    ...
    if (verificationRepository.countByUserIdAndLastSentAtGreaterThanEqual(userId, today)
            >= DAILY_SEND_LIMIT
        || verificationRepository.countByPhoneNumberAndLastSentAtGreaterThanEqual(
                phoneNumber, today)
            >= DAILY_SEND_LIMIT) {
      throw new BusinessException(
          HttpStatus.TOO_MANY_REQUESTS, "Daily verification send limit exceeded.");
    }
  }
  ...
  public record PhoneVerificationStatusView(
      boolean phoneVerified, String maskedPhoneNumber, Instant verifiedAt) {}
  ...
  public record ConfirmPhoneVerificationResult(
      boolean phoneVerified, String maskedPhoneNumber, String nextPath) {}
}
