...
public class AdminMatchingOperationsService {
  ...
  public MatchingOperationsView findOperations(LocalDate date) {
    ...
    List<MatchProposal> matches =
        matchProposalRepository
            .findByMatchedAtGreaterThanEqualAndMatchedAtLessThanOrderByMatchedAtDescIdDesc(
                from, to);
    ...
    List<EligibleUserView> unmatchedUsers =
        eligibleUsers.stream().filter(user -> !matchedUserIds.contains(user.userId())).toList();
  ...
  public ManualMatchResult createManualMatch(ManualMatchCommand command, Long adminId) {
    ...
    matchingPolicy.requirePair(command.userAId(), command.userBId());
    if (command.userAId().equals(command.userBId())) {
      throw new BusinessException(HttpStatus.BAD_REQUEST, "Users must be different.");
    }

    if (!matchingPolicy.eligible(command.userAId()) || !matchingPolicy.eligible(command.userBId()))
      throw new BusinessException(HttpStatus.BAD_REQUEST, "매칭 동의와 휴대폰·사진 심사가 완료된 회원만 소개할 수 있습니다.");
    ...
    if (userA.gender() == null || userA.gender().equals(userB.gender())) {
      throw new BusinessException(
          HttpStatus.BAD_REQUEST, "Manual match requires different genders.");
    }
    if (matchProposalRepository.existsActivePair(
        userA.userId(), userB.userId(), ACTIVE_MATCH_STATUSES)) {
  ...
  public ManualMatchScorePreview previewManualMatchScore(ManualMatchScoreCommand command) {
    ...
    if (matchProposalRepository.existsActivePair(
        candidateA.userId(), candidateB.userId(), ACTIVE_MATCH_STATUSES)) {
        ...
        .orElseThrow(
            () ->
                new BusinessException(
                    HttpStatus.BAD_REQUEST, "User is not eligible for matching."));
  }
  ...
  private List<String> hobbyNames(Long userId) {
    ...
    Map<Long, Hobby> hobbyById =
        hobbyRepository.findAllById(hobbyIds).stream()
