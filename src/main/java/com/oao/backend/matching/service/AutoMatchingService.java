...
public class AutoMatchingService {
  ...
  private static final Set<MatchStatus> ACTIVE_MATCH_STATUSES =
      Set.of(MatchStatus.PENDING, MatchStatus.ACCEPTED);
  ...
  public AutoMatchingResult runAutoMatching() {
    matchingPolicy.lockRun();
    matchingPolicy.expire();
    ...
    while (!unmatched.isEmpty()) {
      ...
      MatchProposal proposal =
          matchProposalRepository.save(
              MatchProposal.createAuto(
                  base.userId(),
                  counterpart.userId(),
                  reason,
                  matchingPolicy.needsGuarantee(base.userId())
                      || matchingPolicy.needsGuarantee(counterpart.userId()),
                  now,
                  expiresAt));
      recordAutoMatch(
          base.userId(),
          counterpart.grade() == com.oao.backend.user.domain.UserAccount.MemberGrade.S);
      recordAutoMatch(
          counterpart.userId(),
          base.grade() == com.oao.backend.user.domain.UserAccount.MemberGrade.S);
      notificationService.notifyMatchArrived(proposal.getId(), base.userId(), counterpart.userId());

      createdMatches.add(
          new CreatedMatchView(
              proposal.getId(),
              base.userId(),
              counterpart.userId(),
              score.totalScore(),
  ...
  private ScoredCounterpart findBestCounterpart(
      MatchingCandidate base, List<MatchingCandidate> candidates, MatchingScoreConfig scoreConfig) {
    return candidates.stream()
        .filter(candidate -> candidate.gender() != base.gender())
        .filter(candidate -> matchingPolicy.pairAllowed(base.userId(), candidate.userId()))
        .filter(
            candidate ->
                !matchingPolicy.needsGuarantee(base.userId())
                    || candidate.grade() == com.oao.backend.user.domain.UserAccount.MemberGrade.S)
        .filter(
            candidate ->
                !matchingPolicy.needsGuarantee(candidate.userId())
                    || base.grade() == com.oao.backend.user.domain.UserAccount.MemberGrade.S)
        .filter(candidate -> !hasActivePair(base.userId(), candidate.userId()))
        .filter(
            candidate ->
                scoreConfig.isAllowPreviousAutoMatch()
                    || !hasPreviousAutoPair(base.userId(), candidate.userId()))
        .map(
            candidate ->
                new ScoredCounterpart(
                    candidate, matchingScoreCalculator.calculate(base, candidate, scoreConfig)))
        .max(
            Comparator.comparingInt(
                    (ScoredCounterpart counterpart) -> counterpart.score().totalScore())
                ...
                .thenComparing(
                    counterpart -> counterpart.candidate().lastAutoMatchedAt(),
                    Comparator.nullsLast(Comparator.reverseOrder()))
                .thenComparing(
                    counterpart -> counterpart.candidate().userId(), Comparator.reverseOrder()))
        .orElse(null);
  }
  ...
  private void recordAutoMatch(Long userId, boolean guaranteed) {
    MatchingProfile matchingProfile =
        matchingProfileRepository
            .findByUserId(userId)
            .orElseGet(() -> matchingProfileRepository.save(MatchingProfile.create(userId)));
    matchingProfile.recordAutoMatch(guaranteed && matchingPolicy.needsGuarantee(userId));
  }
  ...
  public record CreatedMatchView(
      Long matchId, Long userAId, Long userBId, int score, int sharedHobbyCount, String reason) {}
}
